            ...
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    ...
                    <tr class="text-gray-700">
                        ...
                        {{ $notice->noticeTypes ? 'text-' . $notice->noticeTypes->color . '-600' : 'text-gray-600' }}">
                            {{ $notice -> noticeTypes ? ucfirst($notice -> noticeTypes -> type) : 'Unknown' }}
                        </td>
                        <td class="border p-2">
                            @if($notice->expiry_date != null)
                            {{ Carbon:: parse($notice -> expiry_date) -> format('M d, Y h:i A') }}
                            @else
                            <div class="text-red-500">No Expiry</div>
                            @endif
                        </td>
                        <td class="border p-2">{{ Carbon:: parse($notice -> created_at) -> format('M d, Y h:i A') }}
                        </td>
                    </tr>
                    @endforeach
                </tbody>
        ...
        <div class="lg:max-w-6xl mr-auto p-6 my-10 bg-white shadow-lg rounded-md md:mx-auto">
            ...
            <form action="{{ route('admin.notice.store') }}" method="POST" class="space-y-4">
                ...
                <div>
                    ...
                    <span v-if="form.errors.name" class="text-red-500">{{ form.errors.name[0] }}</span>
                </div>
                <div>
                    ...
                    <span v-if="form.errors.description" class="text-red-500">{{ form.errors.description[0]
                        }}</span>
                </div>
                <div>
